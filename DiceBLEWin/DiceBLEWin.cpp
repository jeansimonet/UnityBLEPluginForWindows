...
void SendBluetoothMessage(const char* message)
{
	...
	{
		sendMessageCallback("BluetoothLEReceiver", "OnBluetoothMessage", message);
	}
}
...
void __stdcall HandleBLENotification(BTH_LE_GATT_EVENT_TYPE EventType, PVOID EventOutParameter, PVOID Context)
{
	...
	{
		DebugError(std::string("Received BLE notification for characteristic ").append(BLEUtils::BTHLEGUIDToString(charInfo->characteristic.CharacteristicUuid)).append(" which we did not register with.").c_str());
	}
}
